...
class User(Base):
    ...
    __table_args__ = (
        ...
        # photographer ΓÇö just not twice under the same role. Replaces
    ...
    # directly) ΓÇö kept out of the ORM model entirely so SELECT/INSERT
    # never reference a column that no longer exists.

    # Step 2 ΓÇö auth. Nullable because Google/Apple sign-in accounts have
    # no password at all ΓÇö the provider's ID token is the credential
    ...
    # Step 3 ΓÇö photographer-only studio details (nullable for clients)
    ...
    # Complete Profile step ΓÇö shared by both roles
    avatar_url: Mapped[str | None] = mapped_column(String(500), nullable=True)

    # Studio cover photo (Showcase Portfolio feature) ΓÇö the wide banner
    ...
    # Complete Profile step ΓÇö photographer-only Studio business details.
    ...
    # Studio profile fields (Task 3) ΓÇö photographer ("Studio") role only,
    ...
    # Client optional profile fields (Task 4) ΓÇö client role only, always
    ...
    # Soft delete (Task 10 ΓÇö Delete Account). `is_deleted` is checked by
    # `get_current_user` (app/api/deps.py) so any access token issued
    # before deletion is rejected on its very next use ΓÇö there's no
    ...
    # Email verification (item 21 ΓÇö Verification Pending screen / Resend
    ...
    # Onboarding permission prompts (items 15-17 ΓÇö Camera / Photo Library /
    ...
    # Privacy & Security screen ΓÇö "Download Permissions" toggle. Whether
    ...
    # Privacy & Security screen ΓÇö "Account Privacy" toggle. Used by both
    # studios and clients to control their profile visibility. (The
    # matching "Search Engine Indexing" / "Visibility Preferences" field
    # that used to sit alongside this was removed from the app ΓÇö the
    ...
    # App Settings screen ΓÇö "App Language" picker. Distinct from
    # `languages` above (the studio's spoken-language tags shown on its
    # public profile) ΓÇö this is the user's own UI language preference.
    app_language: Mapped[str] = mapped_column(String(30), default="English", nullable=False)

    # Subscription / billing fields ΓÇö added via SQL migration (not yet in
    ...
    # previously typed as nullable here ΓÇö that mismatch let new-user
    ...
    # Expiry-reminder tracking ΓÇö records the days-remaining threshold the
    ...
    # plan_* columns above) ΓÇö this just makes it visible to SQLAlchemy.
