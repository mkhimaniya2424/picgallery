...
def _invite_existing_client(
    ...
    # Push notification to client's device — gated on the client's own
    # push_notifications_enabled preference (Notification Settings
    # screen); the in-app Notification row above is unaffected.
...
def accept_connection(
    ...
    if connection.initiated_by == ConnectionInitiator.client:
        ...
        db.commit()
        db.refresh(connection)
        # Send live push to client's device — gated on the client's own
        # push_notifications_enabled preference.
        ...
        db.commit()
        db.refresh(connection)
        # Send live push to studio's device — gated on the studio's own
        # push_notifications_enabled preference.
